...
def main():
    ...
    X_train_scaled = scaler.transform(X_train.reshape(-1, len(available_features))).reshape(X_train.shape)
    X_val_scaled = scaler.transform(X_val.reshape(-1, len(available_features))).reshape(X_val.shape)
    X_test_scaled = scaler.transform(X_test.reshape(-1, len(available_features))).reshape(X_test.shape)
    
    # Loaders
    train_loader = DataLoader(TensorDataset(torch.FloatTensor(X_train_scaled), torch.FloatTensor(y_train)), batch_size=BATCH_SIZE, shuffle=False)
